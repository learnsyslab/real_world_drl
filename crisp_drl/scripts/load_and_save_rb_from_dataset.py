...
def main():
    parser = argparse.ArgumentParser(
        description="Loads a lerobot dataset into a replay buffer and saves it to a pickle file."
    )
    parser.add_argument(
        "lerobot_id",
        type=str,
        help="The LeRobot dataset ID (e.g. danielsanjosepro/clean-up-table).",
    )
    parser.add_argument(
        "rb_save_path",
        type=str,
        help="Save location for the replay buffer pickle file.",
    )
    parser.add_argument(
        "--rb_file_name",
        type=str,
        default=None,
        help="File name for the replay buffer pickle file. If not set, uses the lerobot_id as file name.",
    )
    args = parser.parse_args()

    ds = LeRobotDataset(args.lerobot_id)
    rb_file_name = (
        args.lerobot_id.split("/")[-1]
        if args.rb_file_name is None
        else args.rb_file_name
    )
    ...
    #     camera_color_info_topic="/camera/camera/color/camera_info",
    # )
    # manipulator_env_config = FrankaEnvConfig(
    #     max_episode_steps=100,
    #     control_frequency=rlpd_config.control_frequency,
