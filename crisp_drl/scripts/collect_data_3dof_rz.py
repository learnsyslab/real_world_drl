...
env = make_env.create_simulated_env_3dof_rz(
    {
        "initial_keyframe": 2,
        "live_view": False,
    },
...
for p in ps:
    ...
    for i in range(N_ROLLOUTS):
        ...
        while True:
            ...
            perfect_action_rz = (
                ideal_goal_pos_rz - obs["observation.state.cartesian"][5]
            )
            perfect_action_rz = (
                perfect_action_rz
                / np.abs(perfect_action_rz)
                * perfect_action_magnitude_rz
                if np.abs(perfect_action_rz) > 1e-6
                else 0.0
            )
            perfect_action = np.array(
                [perfect_action_xy[0], perfect_action_xy[1], perfect_action_rz]
            )
            ...
            if np.random.rand() < p:
                ...
                action_rz = np.random.uniform(
                    -max_random_action_magnitude_rz,
                    max_random_action_magnitude_rz,
                )
                action = np.array([action_xy[0], action_xy[1], action_rz])
            else:
