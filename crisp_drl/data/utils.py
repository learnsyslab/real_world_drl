...
def crisp_obs_to_tensor(
    ...
    for key, value in observation.items():
        if "image" in key:
            ...
            tensor_list.append(
                encode_image(value, image_encoders[camera_counter], device)
            )
            camera_counter += 1
        elif copy:
...
def encode_image(
    img: np.ndarray, image_encoder: torch.nn.Module, device: torch.device
) -> torch.Tensor:
...
def load_buffer_from_lerobot_dataset(dataset, buffer, num_episodes: int):
    ...
    for batch in dataloader:
        for i in range(batch["action"].shape[0]):
            ...
            current_obs = {
                "observation.state.cartesian": batch["observation.state.cartesian"][i]
                .numpy()
                .copy(),
                "observation.state.gripper": np.expand_dims(
                    batch["observation.state.gripper"][i].numpy(), axis=0
                ).copy(),
                "observation.images.primary": np.transpose(
                    batch["observation.images.primary"][i].numpy() * 255.0, (1, 2, 0)
                ).astype(np.uint8),
                "observation.images.wrist": np.transpose(
                    batch["observation.images.wrist"][i].numpy() * 255.0, (1, 2, 0)
                ).astype(np.uint8),
            }
