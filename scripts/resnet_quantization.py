...
def load_labels(path):
    """Load labels from an ImageNet class-index JSON file and return a mapping
    from synset id (e.g. "n01440764") to the human-readable label
    with spaces replaced by underscores (e.g. "great_white_shark").

    Expected JSON format (imagenet_class_index.json):
    {
        "0": ["n01440764", "tench"],
    ...
    # data maps numeric index strings to [synset_id, human_label]
    for _, pair in data.items():
        assert (
            isinstance(pair, (list, tuple)) and len(pair) == 2
        ), "Invalid label format in JSON file"
        synset, label = pair[0], pair[1]
        img2label[synset] = "_".join(label.split(" "))
...
def evaluate(model, data, target):
    ...
    with torch.no_grad():
        for i, img, target in zip(range(len(data)), data, target):
            start = time.time()
            output = model(img)
            end = time.time()
            delta = end - start
...
class_idx = json.load(open(labels_path))
...
if __name__ == "__main__":
    ...
    model_quantized_gpu = model_quantized.to("cuda")

    print("Evaluating quantized model on the GPU...")
    # Evaluates the quantized model on the test dataset
    inference_time, accuracy = evaluate(model_quantized_gpu, test_data_gpu, test_labels)
    print("Baseline Inference Time: ", inference_time)
    print("Baseline Accuracy: ", accuracy, "%")
