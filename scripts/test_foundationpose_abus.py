...
MESH_PATH_CONTAINER = (
    "/workspaces/isaac_ros-dev/foundation_pose_meshes/abus_key_centered.obj"
)
MESH_PATH_LOCAL = "~/repos/foundation_pose_meshes/abus_key_centered.obj"
...
def main():
    poses_file = "test_images/abus_poses.txt"
    ...
    for idx, (w_T_w_t, w_rpy) in sorted(poses.items()):
        color_path = f"test_images/demo_img_color_{idx}_abus.tiff"
        depth_path = f"test_images/demo_img_depth_{idx}_abus.tiff"
        ...
        output = segmenter.processor.set_text_prompt(
            state=inference_state, prompt="key"
        )
