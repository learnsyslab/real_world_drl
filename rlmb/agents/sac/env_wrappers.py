...
class MoveToBlockWrapper(RewardWrapper):
    def __init__(self, env):
        super().__init__(env)
    ...
    def reward(self, reward, action):
        ...
        """
        eef_pose = self.robot.end_effector_pose
        goal = np.array([0.47, 0.0, 0.08])
        goal_distance = np.linalg.norm(np.array(eef_pose.position) - goal)
        reward = (1/goal_distance + 0.2)
        return reward
...
class SafetyBoundingBoxWrapper(ActionWrapper):
    ...
    def action(self, action):
        ...
        if np.any((action[:3] < -(self.config.max_action + 0.005)) | (action[:3] > self.config.max_action + 0.005)):
